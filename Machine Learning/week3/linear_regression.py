...
class LinearRegressor():
    ...
    def hardcode_bias(self, X):
        ...
        newX = np.concatenate([np.ones((X.shape[0], 1)), X], axis=1)  #Add a column of ones to the left
        ### END CODE
        return newX
    ...
    def fit(self, X, y):
        ...
        ### YOUR CODE HERE 1-3 lines
        newX = self.hardcode_bias(X)
        w = np.linalg.pinv(newX.T @ newX) @ newX.T @ y  #Compute the optimal weights (using w = (X^T X)^-1 X^T y)
        print(w)
        ### END CODE
        self.w =  w
    ...
    def predict(self, X):
        ...
        newX = self.hardcode_bias(X)
        pred = newX @ self.w  #Predict by taking the dot product of newX and the learned weights
        ### END CODE
        return pred
    ...
    def score(self, X, y):
        ...
        ### YOUR CODE HERE 1-3 lines
        pred = self.predict(X)
        score = np.mean((pred - y) ** 2)  #Compute the mean squared error
        ### END CODE
        return score
