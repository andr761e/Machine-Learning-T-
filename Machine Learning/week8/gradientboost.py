...
class GradientBooster():
    ...
    def fit(self, X, y, X_val, y_val, lr=0.1):
        ...
        **Note**: To simplify the implementation, we will **not** require that the first hypothesis you train is a constant function. Instead, just train a regression tree and give it an alpha of 1.
        ...
        val_scores = []
        ### YOUR CODE HERE 
        #Step 1: Initialize the model with a constant value (the mean of the targets)
        y_pred = np.full(y.shape, np.mean(y))
        
        for t in range(self.n_estimators):
          #Step 2: Compute the residuals (negative gradient)
          residuals = y - y_pred
          #y_hat = -2*(self.predict(X) - y)
          
          #Step 3: Train a weak learner (regression tree) on the residuals
          model = self.weak_learner()
          model.fit(X, residuals)
          self.models.append(model)
          
          #Step 4: Update the predictions by adding the weak learner's predictions
          y_pred += model.predict(X)
          
          #Step 5: Append a learning rate alpha of 1 (står i opgaven)
          self.alphas.append(1) 
          
          #Step 6: Track the training and validation loss (mean squared error)
