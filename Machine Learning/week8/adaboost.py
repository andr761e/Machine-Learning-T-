...
class AdaBoostClassifier():
    ...
    def fit(self, X, y):
        ...
        for t in range(self.n_estimators):
          #Train weak learner
          tmp = self.weak_learner()
          tmp.fit(X, y, sample_weight=w)
          y_pred = tmp.predict(X)
          
          #Compute error and alpha (the weight of the weak learner)
          #err = np.dot(w, (y_pred != y).astype(int)) / np.sum(w)
          err = np.sum(w*(y_pred != y))
          #alpha = 0.5 * np.log((1 - err) / (err + 1e-10))  # Add small epsilon to avoid division by zero
    ...
    def exp_loss(self, X, y):
        ...
        ensemble_pred = self.ensemble_output(X)
